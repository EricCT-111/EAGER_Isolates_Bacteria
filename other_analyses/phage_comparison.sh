...
source /path/usr/miniforge3/etc/profile.d/conda.sh
conda activate /path/EAGER_sequences/project_tools/fetch_env

# EDIRECT - to pull viral genomes
efetch -db nuccore -id NC_XXXXX -format fasta > B83.fna

conda deactivate
conda activate /path/EAGER_sequences/project_tools/phage_env
# run pharokka on B83 and BMBtp14
conda deactivate
conda activate path/EAGER_sequences/project_tools/phold_env
# run phold on B83 and BMBtp14

conda deactivate
conda activate /path/EAGER_sequences/project_tools/analysis_env
...
conda activate /path/EAGER_sequences/project_tools/orthofinder_env
